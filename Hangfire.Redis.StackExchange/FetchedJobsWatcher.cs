...
namespace Hangfire.Redis
{
    internal class FetchedJobsWatcher : IServerComponent
    {
        ...
        private void ProcessQueue(string queue, RedisConnection connection)
        {
            ...
            using (new RedisLock(connection.Redis, string.Format(RedisStorage.Prefix + "queue:{0}:dequeued:lock", queue), Thread.CurrentThread.ManagedThreadId.ToString(), _options.FetchedLockTimeout))
            {
                Logger.DebugFormat(
